...
if len(df_final_adi) > MAX_EXCEL_ROWS:
    df_final_adi.head(100_000).to_excel("df_final_adi_100000.xlsx",index=False)
    print(f"엑셀 행 제한 때문에 샘플 100,000행만 저장했습니다. 전체 row 수: {len(df_final_adi)}")
else:
    df_final_adi.to_excel("df_final_adi.xlsx",index=False)
    print(f"전체 저장 완료. row 수: {len(df_final_adi)}")

if len(df_final_oco) > MAX_EXCEL_ROWS:
    df_final_oco.head(100_000).to_excel("df_final_oco_100000.xlsx",index=False)
    print(f"엑셀 행 제한 때문에 샘플 100,000행만 저장했습니다. 전체 row 수: {len(df_final_oco)}")
else:
    df_final_oco.to_excel("df_final_oco.xlsx",index=False)
