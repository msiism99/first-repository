...
if len(df_final_adi) > 0 and 'mrc_fit_x' in df_final_adi.columns:
    ...
    # CSV 저장
    '''
    if len(df_final_adi) > MAX_EXCEL_ROWS:
        df_final_adi.head(1_000_000).to_excel("df_kmrc_decor_adi_1000000.xlsx",index=False)
        print(f"엑셀 행 제한 때문에 샘플 1,000,000행만 저장했습니다. 전체 row 수: {len(df_final_adi)}")
    else:
        df_final_adi.to_excel("df_kmrc_decor_adi.xlsx",index=False)
        print(f"전체 저장 완료. row 수: {len(df_final_adi)}")
    '''
else:
...
if len(df_final_oco) > 0 and 'mrc_fit_x' in df_final_oco.columns:
    ...
    # CSV 저장
    '''
    if len(df_final_oco) > MAX_EXCEL_ROWS:
        df_final_oco.head(1_000_000).to_excel("df_kmrc_decor_oco_1000000.xlsx",index=False)
        print(f"엑셀 행 제한 때문에 샘플 1,000,000행만 저장했습니다. 전체 row 수: {len(df_final_oco)}")
    else:
        df_final_oco.to_excel("df_kmrc_decor_oco.xlsx",index=False)
        print(f"전체 저장 완료. row 수: {len(df_final_oco)}")
    '''
else:
    print("⚠️ KMRC Decorrection 결과가 없습니다.")
...
df_kmrc_adi['lotid'] = df_kmrc_adi['lotid'].astype(str)
df_lotinfo_adi['lotid'] = df_lotinfo_adi['lotid'].astype(str)

df_kmrc_adi['slotid'] = df_kmrc_adi['slotid'].astype(int)
df_lotinfo_adi['slotid'] = df_lotinfo_adi['slotid'].astype(int)

# df_lotinfo_adi에 존재하는 lotid, slotid 조합만 추출
lotinfo_keys = (
    df_lotinfo_adi[['lotid', 'slotid']]
    .dropna()
    .drop_duplicates()
)

df_kmrc_adi = df_kmrc_adi.merge(
    lotinfo_keys,
    on=['lotid', 'slotid'],
    how='inner'
)

print("filtered df_kmrc_adi:", df_kmrc_adi.shape)


df_kmrc_adi.to_excel("df_kmrc.xlsx",index=False)
