...
df_lotinfo_adi_copy = df_lotinfo_adi.copy()

# key type 정리
df_lotinfo_adi_copy['slotid'] = df_lotinfo_adi_copy['slotid'].astype(int)
df_lotinfo_adi_copy['lotid'] = df_lotinfo_adi_copy['lotid'].astype(str)

df_psm_input['lot_id'] = df_psm_input['lot_id'].astype(str)
df_psm_input['sub_eqp_id'] = df_psm_input['sub_eqp_id'].astype(str)

# slotid 홀짝 기준 sub_eqp_id 생성
df_lotinfo_adi_copy['sub_eqp_id'] = np.where(
    df_lotinfo_adi_copy['slotid'] % 2 == 1,
    'EX1',
    'EX2'
)

# df_lotinfo_adi_copy에 존재하는 lotid, sub_eqp_id 조합만 추출
lotinfo_keys = (
    df_lotinfo_adi_copy[['lotid', 'sub_eqp_id']]
    .dropna()
    .drop_duplicates()
    .rename(columns={'lotid': 'lot_id'})
)

# df_psm_input에서 공통 lot_id, sub_eqp_id만 남기기
df_psm_input = df_psm_input.merge(
    lotinfo_keys,
    on=['lot_id', 'sub_eqp_id'],
    how='inner'
)
