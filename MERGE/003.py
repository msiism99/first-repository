...
#if len(df_rawdata_oco) > MAX_EXCEL_ROWS:
#    df_rawdata_oco.head(10_000).to_excel("NAUTIL_DAT_RAWDATA_OCO_10000.xlsx",index=False)
#    print(f"엑셀 행 제한 때문에 샘플 10,000행만 저장했습니다. 전체 row 수: {len(df_rawdata_oco)}")
#else:
#    df_rawdata_oco.to_excel("NAUTIL_DAT_RAWDATA_OCO.xlsx",index=False)
#    print(f"전체 저장 완료. row 수: {len(df_rawdata_oco)}")
...
#if len(df_rawdata_adi) > MAX_EXCEL_ROWS:
#    df_rawdata_adi.head(10_000).to_excel("NAUTIL_DAT_RAWDATA_ADI_10000.xlsx",index=False)
#    print(f"엑셀 행 제한 때문에 샘플 10,000행만 저장했습니다. 전체 row 수: {len(df_rawdata_adi)}")
#else:
#    df_rawdata_adi.to_excel("NAUTIL_DAT_RAWDATA_ADI.xlsx",index=False)
#    print(f"전체 저장 완료. row 수: {len(df_rawdata_adi)}")
