...
# KMRC 데이터 추출 쿼리
# DB_USER: SIMAXP2
# subname: CMP (KMRC Input에 해당)
# df_final에서 photo_transn_seq 추출
# 매칭 키: (photo_transn_seq, lotid, slotid) — 동일 photo_transn_seq라도 slot별로 K값이 다름
# =========================


# ★★★ mstepseq를 꼭 넣어줘야함.  왜냐면 VC075040, VC077251이 섞여서 CMP(=MRC)값이 0인게 뽑히는 경우가 있음 
...
if len(df_final) == 0:
    ...
    df_kmrc = pd.DataFrame()
else:
    # df_final에서 unique photo_transn_seq 추출
    photo_transn_seqs = df_final['photo_transn_seq'].dropna().unique().tolist()
    ...
    df_kmrc = bdq.getData(sql_kmrc)
    print(f"✅ KMRC rows: {len(df_kmrc)}")
    
    if len(df_kmrc) > 0:
        # (photo_transn_seq, lotid, slotid)별 중복 제거 (최신 1건 유지)
        df_kmrc = df_kmrc.sort_values('impala_insert_time', ascending=True)
        df_kmrc = df_kmrc.drop_duplicates(subset=['photo_transn_seq', 'lotid', 'slotid'], keep='last')
        print(f"   중복 제거 후: {len(df_kmrc)} rows (unique photo_transn_seq + lotid + slotid)")
        
        # df_final과의 매칭률 확인 (LOT_ID↔lotid, Wafer↔slotid)
        df_final_keys = set(
            df_final[['photo_transn_seq', 'LOT_ID', 'Wafer']].dropna()
            .apply(lambda r: (r['photo_transn_seq'], r['LOT_ID'], str(r['Wafer'])), axis=1)
        )
        kmrc_keys = set(
            df_kmrc[['photo_transn_seq', 'lotid', 'slotid']]
            .apply(lambda r: (r['photo_transn_seq'], r['lotid'], str(r['slotid'])), axis=1)
        )
        matched = len(df_final_keys & kmrc_keys)
        total = len(df_final_keys)
...
# =========================
# KMRC Decorrection
# calc_regression.py의 kmrc_decorrect, get_coordinates, get_mrc_k_values 로직 참고
# 매칭 키: (photo_transn_seq, lotid, slotid) — slot별로 K값이 다름
# =========================

# -------------------------------------------------------
# 디자인매트릭스 열 ↔ K para 이름 매핑 (design_matrix.py의 coeff_keys 기준)
# 이름 기반 매핑으로 순서 밀림 방지
# -------------------------------------------------------
# X_dx 19열에 대응하는 K para 이름
coeff_keys_dx = [
...
if len(df_kmrc) == 0 or len(df_final) == 0:
    print("⚠️ KMRC 또는 df_final 데이터가 없어 Decorrection을 건너뜁니다.")
else:
    # 필수 컬럼 검증
    missing_cols = [c for c in set(required_dx_cols + required_dy_cols) if c not in df_kmrc.columns]
    if missing_cols:
        print(f"❌ df_kmrc에 필수 컬럼 누락: {missing_cols}")
        print("   KMRC Decorrection을 중단합니다.")
    else:
        print(f"✅ K para 매핑 검증 완료")
        print(f"   X방향 (coeff_keys_dx): {coeff_keys_dx}")
        print(f"   → df_kmrc 컬럼: {required_dx_cols}")
        
        # df_kmrc를 (photo_transn_seq, lotid, slotid)로 인덱싱
        df_kmrc['slotid'] = df_kmrc['slotid'].astype(str)
        kmrc_indexed = df_kmrc.set_index(['photo_transn_seq', 'lotid', 'slotid'])
        
        # df_final의 Wafer도 문자열로 통일
        df_final['_wafer_str'] = df_final['Wafer'].astype(str)
        
        # 결과 저장용 Series
        mrc_fit_x = pd.Series(np.nan, index=df_final.index, dtype=float)
        mrc_fit_y = pd.Series(np.nan, index=df_final.index, dtype=float)
        
        # (photo_transn_seq, LOT_ID, Wafer)별로 처리
        grouped = df_final.groupby(['photo_transn_seq', 'LOT_ID', '_wafer_str'])
        success_count = 0
        skip_count = 0
        ...
        # 6. df_final에 컬럼 추가
        df_final['mrc_fit_x'] = mrc_fit_x
        df_final['mrc_fit_y'] = mrc_fit_y
        df_final['X_reg_demrc'] = df_final['X_reg'] - df_final['mrc_fit_x']
        df_final['Y_reg_demrc'] = df_final['Y_reg'] - df_final['mrc_fit_y']

        
        # raw_x/y: de-MRC'd 값 + PERSHOTMRC
        if 'PSM_X' in df_final.columns and 'PSM_Y' in df_final.columns:
            df_final['raw_x'] = df_final['X_reg_demrc'] - df_final['PSM_X']
            df_final['raw_y'] = df_final['Y_reg_demrc'] - df_final['PSM_Y']


        # 임시 컬럼 제거
        df_final.drop(columns=['_wafer_str'], inplace=True)
        ...
        print(f"   df_final 유효 행: {df_final['mrc_fit_x'].notna().sum()}/{len(df_final)}")

if len(df_final) > 0 and 'mrc_fit_x' in df_final.columns:
    print("=" * 80)
    ...
    print("\n[추가된 컬럼 통계]")
    print(df_final[new_cols].describe())
    
    # 결측치 확인
    print("\n[결측치 확인]")
    for col in new_cols:
        null_count = df_final[col].isna().sum()
        print(f"  {col}: {null_count}개 NaN ({null_count/len(df_final)*100:.1f}%)")
    
    # 보정 전/후 비교
    print("\n[보정 전/후 비교 (처음 5행)]")
    compare_cols = ['photo_transn_seq', 'LOT_ID', 'Wafer', 'X_reg', 'mrc_fit_x', 'X_reg_demrc', 'Y_reg', 'mrc_fit_y', 'Y_reg_demrc']
    available = [c for c in compare_cols if c in df_final.columns]
    display(df_final[available].head())
    
    # CSV 저장
    output_filename = 'BDQ_RawData-1_demrc.csv'
