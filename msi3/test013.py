...
if len(df_psm_input) > 0 and len(df_final) > 0:
    # FCP 좌표 반올림 (소수점 2자리)
    df_psm_input['fcp_x_round'] = df_psm_input['pos_x_valn'].round(2)
    df_psm_input['fcp_y_round'] = df_psm_input['pos_y_valn'].round(2)

    df_final['fcp_x_round'] = df_final['fcp_x'].round(2)
    df_final['fcp_y_round'] = df_final['fcp_y'].round(2)
    ...
    df_final['match_key'] = (
        df_final['apc_hist_index_no'].astype(str) + '_' +
        df_final['fcp_x_round'].astype(str) + '_' +
        df_final['fcp_y_round'].astype(str)
    )
    ...
    final_null = df_final['apc_hist_index_no'].isna().sum()
    if psm_null > 0 or final_null > 0:
        print(f"⚠️ apc_hist_index_no NaN: df_psm_input={psm_null}, df_final={final_null}")
else:
    print("데이터가 없어 매칭 키를 생성할 수 없습니다.")


df_psm_input.to_csv('df_psm_input_2.csv')
df_final.to_csv('df_final_2.csv')

# PSM Fitting 실행
if len(df_final) > 0 and len(df_psm_input) > 0:
    print("PSM Fitting 시작...")
    df_final = shot_fitting(df_final, df_psm_input, k_col_prefix="mrc_k", result_prefix="psm_fit")
    print("PSM Fitting 완료!")
else:
    print("데이터가 없어 PSM Fitting을 수행할 수 없습니다.")
