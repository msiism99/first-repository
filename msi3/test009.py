...
if len(df_result) > 0:
    # TEST 번호에 따라 E1/E2/E3 그룹 분류
    # TEST 1~80: E1, 81~160: E2, 161~240: E3
    df_result['GROUP'] = pd.cut(
        df_result['TEST'], 
        bins=[0, 80, 160, 240], 
        labels=['E1', 'E2', 'E3']
    )
    
    print("✅ GROUP 컬럼 생성 완료")
...
if len(df_result) > 0:
    ...
    for c in cols:
        df_result[c] = pd.to_numeric(df_result[c].astype(str).str.strip(), errors="coerce")

    # Field Center Position 계산
    df_result["fcp_x"] = df_result["DieX"] * df_result["STEP_PITCH_X"] + df_result["MAP_SHIFT_X"]
    df_result["fcp_y"] = df_result["DieY"] * df_result["STEP_PITCH_Y"] + df_result["MAP_SHIFT_Y"]
    
    # Wafer 좌표 계산
    df_result['wf_x'] = df_result['fcp_x'] + df_result['coordinate_X']
    df_result['wf_y'] = df_result['fcp_y'] + df_result['coordinate_Y']
    
    # Radius 계산 (웨이퍼 중심으로부터의 거리)
    df_result['radius'] = np.sqrt(df_result['wf_x']**2 + df_result['wf_y']**2)
...
if len(df_result) == 0:
    print("⚠️ df_result가 비어있어 UNIQUE_ID를 생성하지 않습니다.")
else:
    ...
    def _get_series(col, default=""):
        ...
        if col not in df_result.columns:
            return pd.Series([default] * len(df_result), index=df_result.index, dtype="string")
    ...
    missing_cols = [c for c in required_cols if c not in df_result.columns]
    ...
    if missing_cols:
        ...
        df_result['UNIQUE_ID3'] = base
        df_result['UNIQUE_ID']  = _join_np([base, group], sep="_")
        df_result['UNIQUE_ID2'] = _join_np([base, test, diex, diey, group], sep="_")
        df_result['UNIQUE_ID4'] = _join_np([base, diex, diey, group], sep="_")
...
if len(df_result) > 0:
    ...
    # 필요한 컬럼만 numpy로
    x  = df_result['fcp_x'].to_numpy(dtype=float)
    y  = df_result['fcp_y'].to_numpy(dtype=float)
    sx = df_result['STEP_PITCH_X'].to_numpy(dtype=float) / 2.0
    sy = df_result['STEP_PITCH_Y'].to_numpy(dtype=float) / 2.0
    ...
    df_result['Max_Corner_Distance'] = max_d
    df_result['Is_Edge_Shot'] = max_d > wafer_radius

    print("✅ Edge Shot 판정 완료(벡터화)")
    print(f"Edge Shot 비율: {df_result['Is_Edge_Shot'].mean() * 100:.2f}%")
else:
    print("⚠️ df_result가 비어있어 Edge Shot 판정을 생략합니다.")
...
if len(df_result) > 0:
    ...
    # 존재하는 컬럼만 선택
    available_columns = [col for col in desired_columns if col in df_result.columns]
...
if len(df_final) > 0:
    output_filename = 'BDQ_RawData-1.csv'
    df_final.to_csv(output_filename, index=False, encoding='utf-8-sig')
    
    print("=" * 80)
    print(f"✅ CSV 파일 저장 완료: {output_filename}")
    print(f"   - 총 {len(df_final)} rows, {len(df_final.columns)} columns")
    print("=" * 80)
else:
